...
void Material::setDirLightsUniform(const std::vector<DirLight*>& lights) {
	for (int i = 0; i < std::min(5, (int)lights.size()); i++) {
		setUniform("dirLights[" + std::to_string(i) + "].direction", lights[i]->getForwardVector());
		setUniform("dirLights[" + std::to_string(i) + "].color", lights[i]->color);
		setUniform("dirLights[" + std::to_string(i) + "].intensity", lights[i]->intensity);
	}
	setUniform("numDirLights", std::min(10, (int)lights.size()));
}
